import { ModalTooltip } from "@/app/components/tooltip/tooltip";
import { Tesis } from "./thesis/block";
import Link from "next/link";

export const thesesList: Tesis[] = [
    {
        capture: "Финансы",
        description: <>
            Вы всегда видите реальное состояние бизнеса: баланс, доходы и расходы, долги. Ошиблись в проводке? Не нужно ничего удалять — достаточно сделать сторно. Вы знаете, кто из сотрудников принёс деньги, а кто создал расход. Аналитика помогает вовремя заметить кассовый разрыв.
        </>
    },
    {
        capture: "Сотрудники",
        description: <>
            Сотрудник появляется в отчётности сразу — не нужно ждать регистрации в системе. Назначили должность — права доступа подтянулись автоматически. Уволился? Деактивируйте карточку, данные не теряются. Привязали аккаунт — сотрудник работает сам. Не привязали — вы ведёте учёт за него. Полный контроль даже без компьютеров у сотрудников.
        </>
    },
    {
        capture: "Контрагенты",
        description: <>
            Ведите базу поставщиков, подрядчиков и партнёров. Юрлица, ИП, физлица, банки — каждый со своими реквизитами и историей. Ввели ИНН или название — данные подтянулись автоматически через DaData. Привязали к поставкам и отгрузкам или операциям — видите полную историю взаиморасчётов с каждым контрагентом.
        </>
    },
    {
        capture: "Каталог & Склад",
        description: <>
            Стройте ассортимент удобно: категории, единицы измерения, цены. Учитывайте товары поштучно или партиями (FIFO/LIFO). История поставок и отгрузок даёт точный остаток в реальном времени. Видите, что залёживается, а что уходит — вовремя корректируете закупки.
        </>
    },
    {
        capture: "Клиенты",
        description: <>
            Вся база клиентов с историей и статусами в одном месте. Знаете, откуда пришёл каждый клиент — оцениваете эффективность каналов привлечения. Отслеживаете динамику: новые, активные, ушедшие. Планируете продажи на реальных данных.
        </>
    },
    {
        capture: "Сделки",
        description: <>
            Ведите заказы в канбане от первого обращения до закрытия. В карточке всё вместе: клиент, состав, ответственные, финансы. Меняете состав заказа за пару кликов. Каждая сделка автоматически влияет на финансовую отчётность — без двойного ввода.
        </>
    }
];