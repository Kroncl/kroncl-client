...
import { hrmModule } from '@/apps/company/modules/hrm/api';
...
const REPORT_TYPES = [
    { 
        value: 'employees', 
        label: 'Сотрудники',
        description: 'Список всех сотрудников с контактной информацией'
    },
    { 
        value: 'positions', 
        label: 'Должности',
        description: 'Справочная информация о должностях и их разрешениях'
    }
];
...
export default function Page() {
    ...
    const hrm = hrmModule(companyApi);
    const storageMedia = storageMediaModule(companyApi);
    const router = useRouter();
    const { showMessage } = useMessage();

    const ALLOW_PAGE = usePermission(PERMISSIONS.HRM_REPORT);
    
    const [isLoading, setIsLoading] = useState(false);
    const [selectedTypes, setSelectedTypes] = useState<string[]>(['employees']);
    ...
    const handleSubmit = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await hrm.generateReport({
                types: selectedTypes,
            ...
            if (response.status) {
                ...
                router.push(`/platform/${companyId}/hrm/reports`);
            } else {
    ...
    if (!ALLOW_PAGE.allowed) return (
        <PlatformNotAllowed permission={PERMISSIONS.HRM_REPORT} />
    );
