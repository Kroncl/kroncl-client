...
import { sections } from '../(main)/_sections';
...
export default function Page() {
    ...
    const ALLOW_REPORT = usePermission(PERMISSIONS.HRM_REPORT);
    ...
    return (
        <>
            <PlatformHead
                title='HRM отчёты'
                description='Excel-отчёты по сотрудникам и должностям.'
                searchProps={{
                ...
                actions={isAllowed(ALLOW_REPORT) ? [
                    {
                        ...
                        variant: 'accent',
                        as: 'link',
                        href: `/platform/${companyId}/hrm/reports/new`
                    }
                ] : undefined}
                docsEscort={{
            ...
            {docsList.length === 0 ? (
                <PlatformEmptyCanvas 
                    ...
                    description='Здесь будут появляться сгенерированные HRM отчёты.'
                    icon={<Folder />}
                />
            ) : (
                <>
